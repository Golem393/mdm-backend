...
def get_profile_by_customer(customer_id: str) -> Optional[dict]:
    """Look up a profile by Stripe customer id — the only handle a webhook retry has
    before the Supabase user id is known."""
    client = supabase_admin
    if not client:
        return None
    res = (
...
def verify_user_credentials(email, password):
    ...
    if status not in ("active", "trialing"):
        raise Exception("Active subscription or free trial required. Please manage your plan.")
